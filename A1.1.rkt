...
(module+ test
  ...
  (check-equal? (eva '(add1 5)) 6)
  (check-equal? (eva '((λ (x) ((λ (x) (add1 x)) x)) 0)) 1)
  (check-equal? (eva '((λ (x) ((λ (x) ((λ (x) (add1 x)) x)) x)) 0)) 1)
  (check-equal? (eva '((λ (x) ((λ (y) ((λ (x) (add1 x)) y)) x)) 0)) 1)

  (check-equal? (sub 'x 0 '(λ (y) (add1 x))) '(λ (y) (add1 0)))
  (check-equal? (sub 'x 0 '(add1 x)) '(add1 0))
...
(define (eva term)
  (match term
    [`((λ (,id) (,f-term ,id)) ,v) `,((λ (id) (eva `(,f-term ,v))) v)]
    [`(add1 ,a-term) `,(add1 a-term)]
    [_ term])
  )
